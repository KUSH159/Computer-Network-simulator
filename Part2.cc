...
static void
 ...
 TraceHyblaCwnd ()
 {
   ...
   cWndHyblaStream = ascii.CreateFileStream ("BusyTcpHybla-cwnd.data");
...
//intitializing output stream object for storing congestion window data of TCP Westood
 ...
 {
   AsciiTraceHelper ascii;
   cWndWestwoodStream = ascii.CreateFileStream ("BusyTcpWestwood-cwnd.data");
 ...
 {
   AsciiTraceHelper ascii;
   cWndYeahStream = ascii.CreateFileStream ("BusyTcpYeah-cwnd.data");
...
int main(int argc, char *argv[])
{
	...
	NodeContainer netr (r1, r2);
	NodeContainer net3 (r2, n3);
	NodeContainer net4 (r2, n4);
	...
	p2p.SetQueue ("ns3::DropTailQueue", "MaxSize", QueueSizeValue (QueueSize ("30p")));
	...
	serverApps3.Stop(Seconds(200.0));

	serverApps4.Start (Seconds(1.0));
	serverApps4.Stop(Seconds(200.0));

	serverApps5.Start (Seconds(1.0));
	serverApps5.Stop(Seconds(200.0));


	clientApp0.Start(Seconds(352.0));
	clientApp0.Stop(Seconds(397.0));

	clientApp1.Start(Seconds(350.0));
	clientApp1.Stop(Seconds(350));

	clientApp2.Start(Seconds(352.0));
	clientApp2.Stop(Seconds(350));
	...
	clientApp1.Start(Seconds(10.0));	clientApp1.Stop(Seconds(150.0));

	clientApp2.Start(Seconds(20.0));	clientApp2.Stop(Seconds(197.0));
    ...
    Simulator::Schedule (Seconds (10.00001), &TraceWestwoodCwnd);
    Simulator::Schedule (Seconds (20.00001), &TraceYeahCwnd);
	...
	FlowMonitorHelper flowmon;
  	...
  	throughputHyblaStream = throughput.CreateFileStream ("BusyTcpHybla-throughput.data");
  	throughputWestwoodStream = throughput.CreateFileStream ("BusyTcpWestwood-throughput.data");
  	throughputYeahStream = throughput.CreateFileStream ("BusyTcpYeah-throughput.data");
  	...
  	goodputHyblaStream = throughput.CreateFileStream ("BusyTcpHybla-goodput.data");
  	goodputWestwoodStream = throughput.CreateFileStream ("BusyTcpWestwood-goodput.data");
  	goodputYeahStream = throughput.CreateFileStream ("BusyTcpYeah-goodput.data");
	...
	Simulator::Schedule (Seconds (2.00001), &putTracer, monitor, classifier, 120.0, 150.0, 197.0);
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
     ...
     std::cout<<"Max throughput for TCP Hybla: "<< mx_throughput_hybla <<"Kbps\n";
     std::cout<<"Max throughput for TCP Westwood: "<< mx_throughput_westwood <<"Kbps\n";
     std::cout<<"Max throughput for TCP Yeah: "<< mx_throughput_yeah <<"Kbps\n";
	Simulator::Destroy();
	return 0;
